 ...
 */
public abstract class AbstractGrabServiceAdapter extends AbstractDubboServiceAdapter {

    protected <T> void validateResponse(SaasResult<T> result, RpcActionEnum action, Object... args) {
        super.validateResponse(result, action, args);

        if (!result.isSuccess()) {
            throw new RpcServiceException("[MERCHANT] Error server! errorMsg: " + result.getStatusText() + " - action: " + action + appendArgs(args));
        }
    }
     ...
     */
    protected <T> void validateResponseEntity(SaasResult<T> result, RpcActionEnum action, Object... args) {
